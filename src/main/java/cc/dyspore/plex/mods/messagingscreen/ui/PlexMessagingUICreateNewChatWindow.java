...
public final class PlexMessagingUICreateNewChatWindow extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float par3) {
		...
		int headerSize = PlexUtilRender.drawCenteredTextWrapScaledString(this.headerText, this.startX + this.getWidth() / 2, this.startY, (int) (this.getWidth() * 0.75F),1.5F, this.headerColour, 2);
		int footerSize = 0;
		String description = "";
		if (selectedChannelType != null) {
			description = selectedChannelType.description;
			footerSize = PlexUtilRender.calculateCenteredTextWrapScaledStringHeight(description, (int) (this.getWidth() * 0.75F),1.0F, 1);
		}
		int footerLocation = this.endY - footerSize - 1;

		this.updateChannelSelectionPosition(headerSize, footerSize);
		this.channelSelection.drawScreen(mouseX, mouseY, par3);
		PlexUtilRender.drawCenteredTextWrapScaledString(description, this.startX + this.getWidth() / 2, footerLocation, (int) (this.getWidth() * 0.75F),1.0F, this.descriptionColour, 1);
	}
