package cc.dyspore.plex.mods.messagingscreen.ui;
...
import cc.dyspore.plex.mods.messagingscreen.render.PlexMessagingMessageRenderState;
import cc.dyspore.plex.mods.messagingscreen.render.PlexMessagingMessageTextData;
...
import cc.dyspore.plex.ui.widget.PlexUIScrollbar;
...
public final class PlexMessagingUIMessageWindow extends GuiScreen {
	...
	// chat element scaling - for your custom experience (tm)
	// ###########################################
	...
	public float maxSystemMessageWidthPercent = 0.65F;

	public float messagePaddingPercentX = 0.0F;
	public float messagePaddingPercentY = 0.125F;

	public float messageTopShiftPercent = 0.115F;

	public int extraMessagePaddingX = 2;
	public int extraMessagePaddingY = 0;

	public int messageAuthorSeparatorSize = 1;
	...
	public int getXPaddingByScaledTextHeight(int textHeight, float scale) {
		return (int) (textHeight * scale * this.messagePaddingPercentX) + this.extraMessagePaddingX;
	}

	public int getYPaddingByScaledTextHeight(int textHeight, float scale) {
		return (int) (textHeight * scale * this.messagePaddingPercentY) + this.extraMessagePaddingY;
	}

	public int getXPaddingByTextHeight(int textHeight) {
		return this.getXPaddingByScaledTextHeight(textHeight, 1.0F);
	}
	
	public int getYPaddingByTextHeight(int textHeight) {
		return this.getYPaddingByScaledTextHeight(textHeight, 1.0F);
	}
	
	public int getMessageTopShiftByScaledTextHeight(int textHeight, float scale) {
		return (int) (textHeight * scale * this.messageTopShiftPercent);
	}

	public int getDefaultBackdropSizeByScaledTextHeight(int textHeight, float scale) {
		return (int) ((textHeight * scale) + (this.getYPaddingByScaledTextHeight(textHeight, scale) * 2));
	}
	...
	public PlexMessagingMessageRenderData calculateMessageRenderData(PlexMessagingMessage message, PlexMessagingMessageRenderState renderState) {
		...
		if (message.type == message.TYPE_CHAT_MESSAGE) {
			...
			int authorExtra = renderState.RENDER_AUTHOR_ENABLED ? (int)(Plex.minecraft.fontRendererObj.FONT_HEIGHT * this.authorTextScale) + this.messageAuthorSeparatorSize : 0;
			int authorWidth = renderState.RENDER_AUTHOR_ENABLED ? PlexUtilRender.calculateScaledStringWidth(message.author, this.authorTextScale) : 0;
			...
			for (String line : textLinesSplit) {
				...
				List<String> textWrapLines = PlexUtilRender.textWrapScaledString(line, this.getMaxChatMessageWidth() - (this.getXPaddingByScaledTextHeight(Plex.minecraft.fontRendererObj.FONT_HEIGHT, this.messageTextScale) * 2) - playerHeadExtra, this.messageTextScale);
			...
			int y = authorExtra + this.getYPaddingByScaledTextHeight(Plex.minecraft.fontRendererObj.FONT_HEIGHT, this.messageTextScale) + this.getMessageTopShiftByScaledTextHeight(Plex.minecraft.fontRendererObj.FONT_HEIGHT, this.messageTextScale);
			for (int lineNo = 0; lineNo < textLines.size(); lineNo++) {
				int lineSize = PlexUtilRender.calculateScaledStringWidth(textLines.get(lineNo), this.messageTextScale); // text width
				lineSize += (this.getXPaddingByScaledTextHeight(Plex.minecraft.fontRendererObj.FONT_HEIGHT, this.messageTextScale) * 2); // padding
				renderData.addTextLine(textLines.get(lineNo), this.messageTextScale, this.getXPaddingByScaledTextHeight(Plex.minecraft.fontRendererObj.FONT_HEIGHT, this.messageTextScale) + (message.position == message.POSITION_LEFT ? playerHeadExtra : 0), y, lineSize, message.getColour(), lineCharPos.get(lineNo));
			...
			backdropHeight += this.getYPaddingByScaledTextHeight((int) (Plex.minecraft.fontRendererObj.FONT_HEIGHT * this.messageTextScale), this.messageTextScale) * 2; // top and bottom padding
			...
			for (String line : textLinesSplit) {
				charPos++; // account for newline
				List<String> textWrapLines = PlexUtilRender.textWrapScaledString(line, this.getMaxSystemMessageWidth() - (this.getXPaddingByScaledTextHeight(Plex.minecraft.fontRendererObj.FONT_HEIGHT, this.messageTextScale) * 2), this.messageTextScale);
				for (String wrapLine : textWrapLines) {
					textLines.add(wrapLine);
					lineCharPos.add(charPos);
					charPos += wrapLine.length();
...
/*	public List<PlexMessagingArbitraryMessageState> getArbitraryRenderState(List<PlexMessagingMessage> messages) {
		int currentY = 0;
		boolean headsEnabled = this.messageListContainsHeads(messages);
		for (int i = 0; i < messages.size(); i++) {
			PlexMessagingMessage previousMessage = i - 1 >= 0 ? messages.get(i - 1) : null;
			PlexMessagingMessage message = messages.get(i);
			PlexMessagingArbitraryMessageState messageState = new PlexMessagingArbitraryMessageState(message);
			messageState.renderState.setHeadsShown(headsEnabled);
			boolean author = false;
			if (message.type == message.TYPE_SYSTEM_MESSAGE) {
				messageState.VALID_ITEM = true;
				messageState.renderState.setAuthorEnabled(false);
				messageState.renderState.setHeadEnabled(false);
			...
			if (message.type == message.TYPE_CHAT_MESSAGE) {
				...
				messageState.renderState.setAuthorEnabled(author);
				messageState.renderState.setHeadEnabled(author);
				currentY = this.addToNonZero(currentY, author ? this.messageSpacingDifferentAuthor : this.messageSpacingSameAuthor);
			}
			if (messageState.VALID_ITEM) {
	...
	public PlexMessagingMessageHoverState getMessageHoverState(PlexMessagingMessage message, int positionY, PlexMessagingMessageRenderState messageState, int mouseX, int mouseY) {
		...
		if (renderData.authorVisible && messageState.RENDER_AUTHOR_ENABLED) {
			...
			int authorWidth = PlexUtilRender.calculateScaledStringWidth(renderData.authorName, renderData.authorScale);
		...
		for (PlexMessagingMessageTextData line : renderData.textLines) {
			...
			int wordSpaceWidth = PlexUtilRender.calculateScaledStringWidth(" ", line.scale);
			int wordX = lineX;
			String builtLine = "";
			int offset;
			for (Character letter : line.text.toCharArray()) {
				...
				int wordWidth = PlexUtilRender.calculateScaledStringWidth(builtLine, line.scale);
	...
	public int drawMessage(PlexMessagingMessage message, int positionY, PlexMessagingMessageRenderState messageState) {
		...
		if (renderData.playerHead != null && messageState.RENDER_HEAD_ENABLED) {
			...
			if (headY + renderData.playerHeadSize > this.getRenderBorderTop() && headY < this.getRenderBorderBottom()) {
				PlexUtilRender.drawPlayerHead(renderData.playerHead, headX, headY, renderData.playerHeadSize);
			}
		}
		if (renderData.displayBackdrop) {
			int bdStartX = renderData.getItemXPosition(this.getChatStartX(), this.getChatEndX(), renderData.textBackdropX);
			...
			PlexUtilRender.drawGradientRect(bdStartX, bdTop, bdEndX, bdBottom, renderData.backdropColour, renderData.backdropColour);
		}
		if (renderData.authorVisible && messageState.RENDER_AUTHOR_ENABLED) {
			int authorX = renderData.getItemXPosition(this.getChatStartX(), this.getChatEndX(), renderData.authorX);
			int authorY = renderData.getItemYPosition(positionY, renderData.authorY);
			if (authorY + (Plex.minecraft.fontRendererObj.FONT_HEIGHT * renderData.authorScale) > this.getRenderBorderTop() && authorY < this.getRenderBorderBottom()) {
				PlexUtilRender.drawScaledString(renderData.authorName, authorX, authorY, 0xffffff, renderData.authorScale, false);
			}
		}
		for (PlexMessagingMessageTextData line : renderData.textLines) {
			int lineX = renderData.getItemXPosition(this.getChatStartX(), this.getChatEndX(), line.x);
			...
			if (lineY + (Plex.minecraft.fontRendererObj.FONT_HEIGHT * line.scale) > this.getRenderBorderTop() && lineY < this.getRenderBorderBottom()) {
				PlexUtilRender.drawScaledString(line.text, lineX, lineY, line.colour, line.scale, false);
			}
			//Plex.renderUtils.drawScaledHorizontalLine(lineX, lineX + line.width, lineY, 0.5F, 0xffffffff);
			//Plex.renderUtils.drawScaledHorizontalLine(lineX, lineX + line.width, lineY + line.getHeight(), 0.5F, 0xffffffff);
		}
