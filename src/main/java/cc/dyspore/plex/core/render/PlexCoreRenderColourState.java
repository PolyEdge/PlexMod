package cc.dyspore.plex.core.render;
...
public class PlexCoreRenderColourState {
	...
	public int getRealColour(int colour) {
		return colour == -1 ? PlexUtilColour.globalChromaCycle() : colour;
	}
	...
	public Integer colourBetween(Integer colour1, Integer colour2, Float between, Integer def) {
		between = between == null ? 1.0F : PlexUtil.clamp(between, 0.0F, 1.0F);
		...
		return PlexUtilColour.between(this.getRealColour(colour1), this.getRealColour(colour2), between);
	}
	...
	public Integer colourBetweenStates(String colour, PlexCoreRenderColourState state1, PlexCoreRenderColourState state2, Float between, Integer def) {
		...
		return PlexUtilColour.between(state1.getColour(colour, def), state2.getColour(colour, def), between);
	}
