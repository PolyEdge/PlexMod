...
public class PlexUIScrolledItemList extends GuiScreen {	
	...
	public float getScrollLocationOfItem(PlexUIScrolledItem item) {
		...
		return PlexUtil.clamp((float) ypos / (float) scrollRange, 0.0F, 1.0F);
	}
		...
		for (PlexUIScrolledItem item : itemsSearch) {
			...
			if (currentY + heightOrDefault(item) < viewportTop - this.renderBorderTop) {
				if (this.renderBorderTransition && currentY + heightOrDefault(item) >= viewportTop - this.renderBorderTop - this.renderBorderTransitionDistance) {
					alpha = 1.0F - PlexUtil.clamp(Math.abs((float)((viewportTop - this.renderBorderTop) - (currentY + heightOrDefault(item)))) / (float)this.renderBorderTransitionDistance, 0.0F, 1.0F);
				}
				else {
					continue;
				}
			...
			if (currentY > viewportBottom + this.renderBorderBottom) {
				...
				//	alpha = 1.0F - PlexUtil.clamp(Math.abs((float)((viewportBottom + this.renderBorderBottom) - currentY)) / (float)this.renderBorderTransitionDistance, 0.0F, 1.0F);
				//	PlexUtilRender.drawScaledString("" + alpha, 150.0F, 60.0F, 0xffffff, 0.75F, false);
				//	PlexUtilRender.drawScaledString("" + (viewportBottom + this.renderBorderBottom) , 150.0F, 66.0F, 0xffffff, 0.75F, false);
				//	PlexUtilRender.drawScaledString("" + currentY, 150.0F, 72.0F, 0xffffff, 0.75F, false);
				//}
				//else {
				//	break;
				//}
			...
			if (isMouseOver) {
				...
				itemBackgroundColour = PlexUtilColour.multiply(itemBackgroundColour, 1.20F);
			}

			itemBackgroundColour = PlexUtilColour.multiply(itemBackgroundColour, 1.0F, 1.0F, 1.0F, alpha);

			
			PlexUtilRender.drawGradientRect(this.startX, itemYposition, this.endX, itemYposition + heightOrDefault(item), itemBackgroundColour, itemBackgroundColour);
	
			if (itemText != null) {
				String finalText = PlexUtilRender.trimScaledTextToWidth(itemText, (this.getEndXWithScrollbar() - this.paddingX) - (this.startX + this.paddingX), 1.0F);
				PlexUtilRender.drawScaledStringLeftSide(finalText, this.startX + this.paddingX, itemYposition + ((heightOrDefault(item) / 2) - (Plex.minecraft.fontRendererObj.FONT_HEIGHT / 2)), itemForegroundColour, 1.0F);
			}
			else {
